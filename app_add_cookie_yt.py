...
def get_encoder_args(encoder, fps, bitrate="5M"):
    ...
    if encoder == "h264_nvenc":
        ...
        return ["-c:v", "h264_videotoolbox", "-allow_sw", "1",
                ...
                "-b:v", bitrate, "-maxrate", "8M", "-bufsize", "10M"]
    else:
...
def _apply_pip_segment_progress(input_path, overlay_paths, overlay_positions,
    ...
    for opath in overlay_paths:
        input_args += ["-stream_loop", "-1", "-an", "-i", opath]
...
class VideoTab(tk.Frame):
    ...
    def _setup_variables(self):
        ...
        self._ytdl_cookie_text = None   # tk.Text widget, set khi build UI
    ...
    def _build_left_column(self, parent):
        ...
        # Cookie paste row
        ck_hdr = tk.Frame(ytdl_opts, bg=CARD_BG)
        ck_hdr.pack(fill="x", pady=(0, 2))
        tk.Label(ck_hdr, text="Cookies (dán nội dung cookies.txt)",
                 bg=CARD_BG, fg=TEXT_SECONDARY, font=(UI_FONT, 9)).pack(side="left", padx=(8, 6))
        tk.Label(ck_hdr, text="để trống nếu không cần",
                 bg=CARD_BG, fg=TEXT_SECONDARY, font=(UI_FONT, 8, "italic")).pack(side="left")
        ck_box_f = tk.Frame(ytdl_opts, bg=CARD_HEADER)
        ck_box_f.pack(fill="x", padx=8, pady=(0, 6))
        _ck_sb = tk.Scrollbar(ck_box_f, orient="vertical", width=10)
        _ck_sb.pack(side="right", fill="y")
        self._ytdl_cookie_text = tk.Text(
            ck_box_f, height=3, bg=CARD_HEADER, fg=TEXT_PRIMARY,
            insertbackground=TEXT_PRIMARY, font=(UI_FONT, 9),
            relief="flat", borderwidth=0, highlightthickness=0,
            wrap="none", yscrollcommand=_ck_sb.set,
        )
        self._ytdl_cookie_text.pack(fill="x", padx=6, pady=4)
        _ck_sb.config(command=self._ytdl_cookie_text.yview)
        ...
        _dl_row = tk.Frame(ytdl_opts, bg=CARD_BG)
        _dl_row.pack(fill="x", pady=(0, 6))
        self.ytdl_btn = FlatButton(_dl_row, "⬇  Tải xuống",
                                   command=self._ytdl_start, height=34)
        self.ytdl_btn.pack(side="left", fill="x", expand=True, padx=(0, 4))
        FlatButton(_dl_row, "⟳ Cập nhật yt-dlp",
                   command=self._ytdl_update, height=34, width=130).pack(side="right")
    ...
    def _ytdl_update(self):
        threading.Thread(target=self._do_ytdl_update, daemon=True).start()

    def _do_ytdl_update(self):
        import subprocess as _sp, sys as _sys
        self.log("Đang cập nhật yt-dlp...", "info")
        r = _sp.run([_sys.executable, "-m", "pip", "install", "-U", "yt-dlp"],
                    capture_output=True, text=True, encoding="utf-8", errors="replace")
        if r.returncode == 0:
            # lấy version mới sau khi cài
            try:
                import importlib, yt_dlp as _yt
                importlib.reload(_yt)
                ver = _yt.version.__version__
            except Exception:
                ver = "?"
            self.log(f"✓ yt-dlp đã cập nhật lên {ver} — khởi động lại app để áp dụng", "success")
        else:
            self.log(f"✗ Cập nhật thất bại: {r.stderr[-300:]}", "error")
    ...
    def _ytdl_download(self):
        ...
        try:
            _ver = yt_dlp.version.__version__
        except Exception:
            _ver = "?"
        self.log(f"yt-dlp {_ver}", "info")
        ...
        # cookie args — ghi nội dung dán vào file tạm (tự detect format)
        _cookie_opts = {}
        _cookie_tmp  = None
        if self._ytdl_cookie_text:
            _ck_content = self._ytdl_cookie_text.get("1.0", "end").strip()
            if _ck_content:
                import tempfile as _tf
                _cookie_tmp = os.path.join(_tf.gettempdir(), "ytdl_cookies.txt")
                # Nếu là Netscape format (có tab hoặc header) → dùng nguyên
                # Nếu là HTTP header format (key=value; key2=value2) → convert
                if "\t" in _ck_content or _ck_content.startswith("# Netscape"):
                    _netscape = _ck_content
                else:
                    # Cookie Google account → phải dùng .google.com, không phải .youtube.com
                    _GOOGLE_CK = {
                    ...
                    for _pair in _ck_content.split(";"):
                        _pair = _pair.strip()
                        if "=" not in _pair:
                            continue
                        _name, _, _val = _pair.partition("=")
                        _name = _name.strip()
                        _val  = _val.strip()
                        _secure  = "TRUE" if _name.startswith("__Secure-") or _name.startswith("__Host-") else "FALSE"
                        _domain  = ".google.com" if _name in _GOOGLE_CK else ".youtube.com"
                        _lines.append(f"{_domain}\tTRUE\t/\t{_secure}\t9999999999\t{_name}\t{_val}")
                    _netscape = "\n".join(_lines)
                with open(_cookie_tmp, "w", encoding="utf-8") as _f:
                    _f.write(_netscape)
                _cookie_opts["cookiefile"] = _cookie_tmp
                _n_ck = _netscape.count("\n")
                self.log(f"[Cookie] Đã tạo {_cookie_tmp} ({_n_ck} dòng)", "info")

        # Android client bypass bot detection; web là fallback cho age-restricted
        _YT_EXTRACTOR_ARGS = {"extractor_args": {"youtube": {"player_client": ["android", "web"]}}}
        ...
        try:
            ...
            if mode == "Link URL":
                ...
                _auto_browsers = (
                    ["safari", "chrome", "firefox", "edge", "chromium"]
                    if _plat.system() == "Darwin" else
                    ["chrome", "firefox", "edge", "chromium"]
                    if _plat.system() == "Windows" else
                    ["chrome", "firefox", "chromium"]
                )

                def _parse_info(info, u):
                ...
                def _extract_url(u):
                    # Thử với opts hiện tại
                    try:
                        with yt_dlp.YoutubeDL(info_opts) as ydl:
                            return _parse_info(ydl.extract_info(u, download=False), u)
                    except Exception as exc:
                        if "Sign in to confirm" not in str(exc) and "bot" not in str(exc).lower():
                            self.log(f"  Lỗi URL {u[:60]}: {exc}", "error")
                            return []

                    # Bot detection → thử từng trình duyệt đã cài
                    for _br in _auto_browsers:
                        try:
                            _br_opts = {**info_opts,
                                        "cookiesfrombrowser": (_br, None, None, None),
                                        **_YT_EXTRACTOR_ARGS}
                            _br_opts.pop("cookiefile", None)
                            with yt_dlp.YoutubeDL(_br_opts) as ydl:
                                info = ydl.extract_info(u, download=False)
                            self.log(f"  [Cookie] OK với trình duyệt: {_br}", "info")
                            # Cập nhật _cookie_opts in-place để download dùng browser này
                            _cookie_opts.clear()
                            _cookie_opts["cookiesfrombrowser"] = (_br, None, None, None)
                            return _parse_info(info, u)
                        except Exception:
                            continue

                    self.log(f"  Lỗi URL {u[:60]}: bot detection — đăng nhập YouTube trong Chrome/Safari rồi thử lại", "error")
                    return []
                ...
                try:
                    ...
                    _search_exc = _e
                if info is None and ("Sign in to confirm" in str(_search_exc) or "bot" in str(_search_exc).lower()):
                    for _br2 in _auto_browsers2:
                        try:
                            _sr_opts = {**search_opts,
                                        "cookiesfrombrowser": (_br2, None, None, None),
                                        **_YT_EXTRACTOR_ARGS}
                            _sr_opts.pop("cookiefile", None)
                            with yt_dlp.YoutubeDL(_sr_opts) as ydl:
                                info = ydl.extract_info(f"ytsearch{max_results}:{keyword}", download=False)
                            self.log(f"  [Cookie] OK với trình duyệt: {_br2}", "info")
                            _cookie_opts.clear()
                            _cookie_opts["cookiesfrombrowser"] = (_br2, None, None, None)
                            break
                        except Exception:
                            continue
                if info is None:
                    raise _search_exc or Exception("Tìm kiếm thất bại")
                videos = [
