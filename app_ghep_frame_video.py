...
class VideoTab(tk.Frame):
    ...
    def _build_left_column(self, parent):
        ...
        tk.Label(
            self.config_card.body, text="Định dạng video",
            bg=CARD_BG, fg=TEXT_SECONDARY, font=(UI_FONT, 9)
        ).pack(anchor="w", pady=(0, 5))

        fmt_field = tk.Frame(self.config_card.body, bg=CARD_HEADER)
...
"""
    ...
    def process(self):
        ...
        try:
            ...
            # STEP 6 (or 7): Burn subtitle song song (nếu có) + Final concat
            t0 = time.time()
            current_step = 6 + step_offset
            sorted_results = [results[idx] for idx in sorted(results.keys())]

            if use_subtitle:
                # ── Bước 6a: Remap subtitle sang timeline mới ──
                ...
                # ── Bước 6b: Phân chia subtitle theo từng segment + viết ASS ──
                seg_cumul = 0.0
                burn_tasks = []
                for pos_i, (orig_idx, _audio_path, seg_dur) in enumerate(final_audio_order):
                    seg_start = seg_cumul
                    ...
                    seg_video = sorted_results[pos_i]
                    burned_path = os.path.join(self.TEMP_FOLDER, f"burned_{pos_i:04d}.mp4")

                    if seg_subs:
                        ass_path = os.path.join(self.TEMP_FOLDER, f"sub_{pos_i:04d}.ass")
                        self.write_ass(seg_subs, ass_path, sub_font_size, width, height)
                    else:
                        ass_path = None

                    burn_tasks.append((pos_i, seg_video, ass_path, burned_path, fps, BEST_ENCODER))
                    seg_cumul += seg_dur

                # ── Bước 6c: Burn song song với progress ──
                self.update_status(f"Bước {current_step}/{total_steps}: Burn phụ đề song song",
                ...
                with ProcessPoolExecutor(max_workers=max_workers) as ex:
                    burn_futures = {
                        ex.submit(burn_subtitle_segment_task, task): task[0]
                        for task in burn_tasks
                    }
                    done_burn = 0
                    for future in as_completed(burn_futures):
                        bi, ok, bpath, berr = future.result()
                        if ok and os.path.exists(bpath) and os.path.getsize(bpath) > 0:
                            burned_results[bi] = bpath
                        done_burn += 1
                        pct = 86 + (done_burn / len(burn_tasks)) * 10
                        self.update_status(
                            f"Bước {current_step}/{total_steps}: Burn phụ đề song song",
                            f"Đã burn {done_burn}/{len(burn_tasks)} đoạn", pct
                        )

                self.log(
                    f"[{current_step}/{total_steps}] Burn {len(burned_results)}/{len(burn_tasks)} segment "
                    f"({time.time()-t0:.1f}s)", "success"
                )

                # ── Bước 6d: Concat các segment đã burn → OUTPUT_VIDEO ──
                t0 = time.time()
                self.update_status(f"Bước {current_step}/{total_steps}: Ghép cuối",
                                   "Concat các đoạn đã burn...", 96)

                burned_list = [burned_results[i] for i in sorted(burned_results.keys())]
                concat_file = os.path.join(self.TEMP_FOLDER, "concat_burned.txt")
                with open(concat_file, "w", encoding="utf-8") as f:
                    for vp in burned_list:
                ...
                result = subprocess.run(merge_cmd, stderr=subprocess.PIPE, text=True,
                                        creationflags=SUBPROCESS_FLAGS)
                if result.returncode != 0:
                    raise Exception(f"Lỗi concat burned segments: {result.stderr}")

                self.log(f"[{current_step}/{total_steps}] Concat cuối xong ({time.time()-t0:.1f}s)", "success")

            else:
                # Không có subtitle: concat thẳng → OUTPUT_VIDEO
                ...
                with open(concat_file, "w", encoding="utf-8") as f:
                    for video in sorted_results:
                        f.write(f"file '{normalize_path(os.path.abspath(video))}'\n")
