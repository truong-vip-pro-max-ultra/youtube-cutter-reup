...
class VideoTab(tk.Frame):
    ...
    def _build_left_column(self, parent):
        ...
        # === YouTube Downloader ===
        ytdl_card = Card(parent, "⬇  Tải YouTube")
        ytdl_card.pack(fill="x", pady=(0, 12), padx=(0, 4))

        ytdl_opts = tk.Frame(ytdl_card.body, bg=CARD_BG)

        def _toggle_ytdl():
            if self.show_ytdl.get():
                ytdl_opts.pack(fill="x")
            else:
                ytdl_opts.pack_forget()

        ToggleSwitch(ytdl_card.body, "Tải audio / video từ YouTube",
                     self.show_ytdl, command=_toggle_ytdl
                     ).pack(anchor="w", fill="x", pady=(0, 4))

        # Keyword
        kw_f = tk.Frame(ytdl_opts, bg=CARD_HEADER)
        kw_f.pack(fill="x", pady=(0, 6))
        tk.Label(kw_f, text="Từ khoá", bg=CARD_HEADER, fg=TEXT_SECONDARY,
                 font=(UI_FONT, 9)).pack(side="left", padx=(10, 6), pady=6)
        tk.Entry(kw_f, textvariable=self.ytdl_keyword,
                 bg=INPUT_BG, fg=TEXT_PRIMARY, insertbackground=TEXT_PRIMARY,
                 font=(UI_FONT, 10), relief="flat", borderwidth=0,
        ...
        ttk.Combobox(type_row, textvariable=self.ytdl_type,
        ...
        tk.Label(audio_opts, text="Định dạng", bg=CARD_BG, fg=TEXT_SECONDARY,
                 font=(UI_FONT, 9)).pack(side="left", padx=(0, 4))
        ttk.Combobox(audio_opts, textvariable=self.ytdl_afmt,
                     values=["mp3", "m4a", "opus", "flac", "wav"],
                     state="readonly", font=(UI_FONT, 9), width=6,
                     style="Modern.TCombobox").pack(side="left", padx=(0, 12))
        tk.Label(audio_opts, text="Chất lượng", bg=CARD_BG, fg=TEXT_SECONDARY,
                 font=(UI_FONT, 9)).pack(side="left", padx=(0, 4))
        ttk.Combobox(audio_opts, textvariable=self.ytdl_aqual,
        ...
        # Numbers row: số lượng | thời lượng max (phút) | luồng
        ...
        for label, var, w in [("Số lượng", self.ytdl_max_res, 4),
            ...
            tk.Label(num_row, text=label, bg=CARD_BG, fg=TEXT_SECONDARY,
                     font=(UI_FONT, 9)).pack(side="left", padx=(8, 4))
            f = tk.Frame(num_row, bg=CARD_HEADER)
            f.pack(side="left", padx=(0, 10))
            tk.Entry(f, textvariable=var, width=w, bg=CARD_HEADER,
                     fg=TEXT_PRIMARY, insertbackground=TEXT_PRIMARY,
                     font=(UI_FONT, 10), relief="flat", borderwidth=0,
                     justify="center").pack(padx=5, ipady=3)

        # Output folder (tuỳ chọn)
        out_f = tk.Frame(ytdl_opts, bg=CARD_HEADER)
        out_f.pack(fill="x", pady=(0, 6))
        tk.Label(out_f, text="Thư mục lưu", bg=CARD_HEADER, fg=TEXT_SECONDARY,
                 font=(UI_FONT, 9)).pack(side="left", padx=(10, 6), pady=6)
        tk.Entry(out_f, textvariable=self.ytdl_out_folder,
                 bg=INPUT_BG, fg=TEXT_PRIMARY, insertbackground=TEXT_PRIMARY,
                 font=(UI_FONT, 10), relief="flat", borderwidth=0,
